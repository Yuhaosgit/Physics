...
#define FLOOR 1
const float Degree_To_Radian = 3.1415926f / 180.0f;
...
const float Friction_coe = 0.3;
const float gravity_coe = 9.8;
...
void PhysicsSystem::BasicCollisionDetection() {
	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				...
				//gain points
				if ((*i)->IsBonus()) {
					if ((*j)->GetWorldID() == MainBall || (*j)->GetName() == "en")
						bonus = *i;
					else
						continue;
				}
				else if ((*j)->IsBonus()) {
					if ((*i)->GetWorldID() == MainBall|| (*i)->GetName() == "en")
						bonus = *j;
					else
						continue;
				}
	...
	if (bonus != nullptr) {
		gameWorld.RemoveGameObject(bonus, true);
		score++;
	}
}
...
void PhysicsSystem::NarrowPhase() {
	...
	GameObject* front = nullptr;
	GameObject* behind = nullptr;
	...
	for (std::set <CollisionDetection::CollisionInfo >::iterator
		...
		if (CollisionDetection::ObjectIntersection(info.a, info.b, info)) {
			...
			behind = info.b;
			//lose game
			if (front->GetWorldID() == MainBall && behind->GetWorldID() == EvilBall)
				gameWorld.SetLoseGame(true);
			else if (front->GetWorldID() == EvilBall && behind->GetWorldID() == MainBall)
				gameWorld.SetLoseGame(true);

			//gain points
			if (front->IsBonus()) {
				if (behind->GetWorldID() == MainBall || (behind)->GetName() == "en") {
					bonus = front;
				}
				else
					continue;
			}
			else if (behind->IsBonus()) {
				if (front->GetWorldID() == MainBall || (front)->GetName() == "en" ) {
					bonus = behind;
				}
				else
					continue;
			...
			//
				...
				//floor friction
				if (front->Type == FLOOR)
					Friction_func(behind, front);
				else if (behind->Type == FLOOR)
					Friction_func(front, behind);
				//
				info.framesLeft = numCollisionFrames;
...
void PhysicsSystem::Friction_func(GameObject* obj, GameObject* floor) {
	Vector3 slope = floor->GetTransform().GetOrientation().ToEuler();
	Vector3 friction_force;
	friction_force = -obj->GetPhysicsObject()->GetLinearVelocity().Normalised() *
		(gravity_coe * floor->friction_coe * 0.1 * std::cosf(slope.z * Degree_To_Radian));
	obj->GetPhysicsObject()->AddForce(friction_force);
	/*
	//gravity division
	if (floor->GetWorldID() == Hill) {
		Vector3 gravity_force;
		gravity_force = Vector3(cosf(slope.z * Degree_To_Radian), sinf(slope.z * Degree_To_Radian), 0)
			.Normalised() * gravity_coe;

		obj->GetPhysicsObject()->AddForce(gravity_force);
	}*/
}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
		...
		(transformA.GetPosition() - (p.normal * p.penetration * (physA->GetInverseMass() / totalMass)));

		transformB.SetPosition
		(transformB.GetPosition() + (p.normal * p.penetration * (physB->GetInverseMass() / totalMass)));
		...
		float cRestitution = a.GetMaterial() * b.GetMaterial(); // disperse some kinectic energy
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	float frameLinearDamping = 1.0f;// -(0.2f * dt);
